...
module.exports = function parse(objectName, ocss) {
  ...
  function hasNoSemicolons(line) {
    if (regex.semicolon.test(line.raw)) {
      error(line, 'no semicolons allowed')
    }
    return true
  }
  ...
  function addType(line) {
    ...
    if (regex.attributeselector.test(trimmedLine))   return attributeselector(line)
  ...
  function attributeselector(line) {
    line.type = 'attributeselector'
    line.name = line.raw.trim()
    return line
  }
